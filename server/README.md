...
> （ヘッダ `x-api-key`、1回で全銘柄）。このWorkerは必要な日付ぶんだけ取得（メモ化で
> 重複回避＝約14日分で**無料プランの50サブリクエスト上限に収まる**）し、各スナップショットの
> **調整後終値（AdjC）と未調整終値（Close）の両方**を保持します。`reconcileChange()` が
> 両者を比較し、通常日は調整後を、乖離時は**変化幅の小さい方**を採用することで、分割
> （未調整側が動く）も調整の不整合（調整後側が動く）も相殺します。これが、日付別スナップ
> ショット同士が権利落ちで食い違って生じた ANA（9202）の見かけ上 1D −33.95% を解消します。
> フィールド名（`AdjC` / `Close` 等）は環境差に強い寛容パースで取り込みます。
