...
>
> V2の日次株価は `GET https://api.jquants.com/v2/equities/bars/daily`（ヘッダ `x-api-key`）。
> このWorkerは各構成銘柄の**連続系列**を `?code=…&from=…&to=…` で1銘柄ずつ取得し、
> 最新値と各期間の基準値を**同一系列**（同一の調整基準）から算出します。日付ごとの
> 市場スナップショットを別々に取ると、権利落ちをまたいで基準が食い違い、見かけ上の
> 急落（例: ANA 9202 の 1D −33.95%）が起きるため、それを避ける設計です。フィールド名
> （`AdjC` 等）は環境差に強い寛容パースで取り込みます。約225銘柄ぶんのサブリクエストを
> 出すため、**Workers 有料（Bundled）プラン**（1000サブリクエスト/リクエスト）を前提とします。
