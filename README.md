...
- **スクリーンショット共有**（左上📷）：カメラマークをクリックすると、いま見ているアングル・ズームのまま画像化。上下に指数名・基準日・サイトURL（`https://3dheatmap.markets-lab.com/`）の帯を焼き込み、**Web Share API**（スマホは画像＋URL付きテキストをそのまま共有）で拡散。PCはダウンロードにフォールバック。`X` / `LINE` / リンクコピーのボタンも併設
