...
- **時系列アニメーション**（下中央）：▶ で**直近5営業日**の日次騰落を順に再生（各日1秒静止＋1秒で次へ遷移＝計9秒）。スライダーで手動スクラブも可能。初期状態は**最新営業日**にセット（▶ で最古から再生）。アニメ中は**面積（寄与度）を各銘柄のウェイトで固定**し、**高さ・色（日次騰落率）だけを動かす**ので銘柄が定位置で拍動して見やすい。期間ボタンを押すと通常表示に戻る
...
- **ローディング演出（ハイパースペース）**：初回のデータ読込中（コールドスタート時は 20〜30 秒
  ほど）に**ハイパースペース航行**のクリップを全画面で流します。**entry（突入）→ cruise（航行・
  読込中はループ）→ exit（通常宇宙へ離脱）**の3フェーズを再生位置で制御し、読込完了で exit を
  再生 → 短いクロスフェードでシーンへ切り替わります。`public/assets/hyperspace.mp4`（H.264・
  ミュート再生・小容量を推奨）を置くと有効化。無ければ従来のスピナー表示にフォールバックします。
  セグメント境界（`SEG`）は `public/js/ui.js` で調整可能。詳細は `public/assets/README.md`。
...
window.HEATMAP_SAMPLE['NIKKEI'] = {
  ...
  // 面積(contribution)は各銘柄のウェイトで全フレーム共通に固定し、
      ...
      { "asOf": "2026-07-22",
        "constituents": [
          { "code": "6857", "name": "アドバンテスト", "sector": "電気機器",
            ...
            "contribution": 68.5 }     // 固定サイズ(ウェイト) → 面積（全フレーム同値）
        ] },
      /* … 古い→新しい 5フレーム … */
    ]
  }
...
TIMELINE の固定サイズ（面積）は各指数のウェイト代理：NIKKEI=`paf×最新終値`、
DOW=`最新終値`（株価加重）、NASDAQ=`weight%`（時価総額加重）。両Worker（日経=J-Quants
