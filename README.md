...
**取得方式（連続系列）**：`server/worker.js` は各構成銘柄の**調整後終値の連続系列**を
J-Quants V2 `equities/bars/daily`（`?code=…&from=…&to=…`）で1銘柄ずつ取得し、
最新値と各期間の基準値を**同一系列**から算出します。日付ごとに市場スナップショットを
別々に取得する旧方式では、コーポレートアクションの権利落ちをまたいで基準日が
**異なる調整基準**に乗ることがあり、ANA（9202）で見かけ上の 1D −33.95% が発生しました。
連続系列にすることで分割・併合が騰落率に混入しません。取得は約225銘柄ぶんのサブリクエスト
（並列度を制限）となり、Cloudflare の **無料プラン50サブリクエスト上限を超える**ため、
このワーカーは **Workers 有料（Bundled）プラン**（1リクエスト1000サブリクエスト）を前提とします。
