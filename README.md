...
**取得方式（無料プラン対応＋調整整合）**：`server/worker.js` は J-Quants V2
`equities/bars/daily?date=YYYYMMDD`（1回で全銘柄）を**必要な日付ぶんだけ**取得し
（日付はメモ化して重複取得を回避＝約14日分でCloudflareのＷorkers **無料プラン50
サブリクエスト上限**に収まる）、最新値と各期間の基準値を突き合わせます。
各スナップショットは**調整後終値（AdjC）と未調整終値（Close）の両方**を保持し、
`reconcileChange()` が両者を比較します：通常日は一致（調整後を採用）、乖離した場合は
**変化幅の小さい方**を採用します。分割は未調整側だけを、調整の不整合（古い調整後終値）は
調整後側だけを大きく動かすため、この方式で**どちらも自動的に相殺**されます。
（この不整合が ANA（9202）の見かけ上の 1D −33.95% の原因でした。）
結果は10分キャッシュされ、コールド更新時のみフル取得コストがかかります。
