...
- **ローディング演出（ハイパースペース）**：初回のデータ読込中（コールドスタート時は 20〜30 秒
  ほど）に**ハイパースペース航行**のクリップを全画面で流し、読込完了の瞬間に**白フラッシュで
  離脱＝この星系に到着**という演出でシーンへ切り替わります。`public/assets/hyperspace.mp4`
  （短いシームレスループの H.264・ミュート再生・小容量を推奨）を置くと有効化。無ければ従来の
  スピナー表示にフォールバックします。詳細は `public/assets/README.md`。
